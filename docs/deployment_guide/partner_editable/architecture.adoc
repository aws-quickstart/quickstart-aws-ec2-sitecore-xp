...
** An external Application Load Balancer (ALB) for routing traffic to Sitecore content and identity role instances in the private subnets.
* In the private subnets:
** Network Load Balancers (NLBs) for Transport Layer Security (TLS) pass-through for the remaining Sitecore instances.
** An internal ALB for Sitecore reporting and processing instances.
